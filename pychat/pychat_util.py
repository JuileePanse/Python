...
class Hall:
    ...
    # Inform all the players in the Hall that a new player just joined.
    def welcome_new(self, new_player):
        new_player.socket.sendall(b'Welcome to pychat.\nPlease tell us your name:\n')

    # To handle the message <list>
    ...
    def handle_msg(self, player, msg):
        ...
        if "name:" in msg:
            ...
            temp_string = msg[7:52].encode()
            parsed_json = json.loads(temp_string)  
            #json.dump(x, sys.stdout)
            temptstr =  str(parsed_json)
            player.socket.sendall(temptstr.encode())
